...
| PostgreSQL  | Extra databases/users, CREATEDB on/off, grants, read-only SQL, dump/restore | Actual project login for SQL/previews; bounded output and timeouts; `sql_execute` requires destructive opt-in; `sql_query` does not. `user_password_rotate` without a password generates and stores one. `database_backup` defaults to `{bucket}-backups`, not the live app bucket. `database_restore` uses that same default, runs as the project login, and requires a `{project}_` extra database name. |
...
| Containers  | List/logs/start/stop/restart/remove by service or container name           | Admin only; no shell or exec. Control-plane (`dashboard`, `gateway`, `agent-broker`, `host-broker`, `docker-proxy`) cannot be stopped or deleted; Wardroom services cannot be deleted. All `container_action` verbs need destructive. |
