...
- The dashboard is a trusted LAN/Tailnet admin tool, with authenticated read-only database browsing and explicit project provisioning. Do not expose the Docker socket or add arbitrary shell/SQL execution. Host telemetry is pushed by the separate telemetry/ user service; reinstall it with make telemetry after collector changes.
